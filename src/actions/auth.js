...
// This stores the authToken in the state and in the local storage
// Also decodes and stores the user data stored in teh token
...
export const login = (username, password) => dispatch => {
  ...
  return (fetch(`${API_BASE_URL}/login`, {
    ...
    // Below will reject any requests which don't return a 200 status, creating
...
export const refreshAuthToken = () => (dispatch, getState) => {
  ...
  return fetch(`${API_BASE_URL}/auth/refresh`, {
    ...
    .then(({authToken}) => storeAuthInfo(authToken, dispatch))
    .catch(err => {
      // Could not get a refresh token because current credentials are invalid or expired
      // or something else went wrong so clear them and sign us out!
      dispatch(authError(err));
      dispatch(clearAuth());
